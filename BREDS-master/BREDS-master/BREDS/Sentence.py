...
class Sentence:
    ...
    def __init__(self, sentence, e1_type, e2_type, max_tokens, min_tokens,
        ...
        for m in re.finditer(entities_regex, sentence):
            print("...... vahab_Sentences:m in re.finditer(entities_regex, sentence)=\n",m)
            entities.append(m)
        print("...... vahab_entities\n",entities)
            #liste entity ha be shekle zir ast
            # [<_sre.SRE_Match object; span=(0, 18), match='<PER>Vaughan</PER>'>, <_sre.SRE_Match object; span=(55, 77), match='<LOC>New Zealand</LOC>'>]
        ...
        if len(entities) >= 2:
            ...
            for i in range(len(sorted_keys)-1):
                ...
                print("=== e1= ", e1.string, e1_type,sorted_keys[i],"   locations=     ",locations[sorted_keys[i]] )
                print("\n=== e2= ", e2.string, e2_type, sorted_keys[i+1],"   locations=  ",locations[sorted_keys[i+1]])
                if max_tokens >= distance >= min_tokens and e1.type == e1_type \
                    ...
                    print("+ + + vahab sentence_no_tags\n", sentence_no_tags)
                    print("+ + + vahab relationship between r= \n", r.between)
                    print ("+ + + vahab relationship befor r= \n", r.before)
                    print("+ + + vahab relationship after r= \n", r.after)
